...
ArduiTouchSmart ats = ArduiTouchSmart(&tft,&tftconf,&AT_Standard9pt7b, false, false);

//Konfigurationsstring für die Widgets
String wdgconf = "{"
  "'label':'Helligkeit',"
  "'topic':'eg/wz/helligkeit',"
  "'valname':'wert',"
  "'unitname':'einheit',"
  "'min':0,'max':5000"
  "}";
...
void setup() {
  ...
  ats.addPage();
  //Widget hinzufügen
  ats.addWidget(0,0,12,10,WT_GAUGE,"hell1",wdgconf.c_str());
  ats.addWidget(0,8,12,4,WT_SLIDER,"slide",sliderconf.c_str());
  ats.addWidget(0,12,6,3,WT_SWITCH,"switch",switchconf.c_str());
  ats.addWidget(6,12,6,3,WT_BUTTON,"btn",buttonconf.c_str());
  //Seite anzeigen
  ats.drawPage();
}
