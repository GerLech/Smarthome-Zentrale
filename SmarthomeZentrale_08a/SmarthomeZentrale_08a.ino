...
#define TFT_LED  15
#define TOUCH_CS 14
#define TOUCH_IRQ 2 //Für AZ-Touch alte Version #define TOUCH_IRQ 2 sonst 27
#define TOUCH_ROTATION 1 //Muss für 2.4 Zoll Display 1 für 2.8 Zoll 3
...
bool publish(const char * thema, const char * message) {
  ...
  sscanf(thema,"%4s/%i",code,&pin);
  Serial.println(code);
  if (strncmp(code,"GPIO",4) == 0) {
    pinMode(pin,OUTPUT);
    sscanf(message,"%i",&val);
    Serial.printf("Set pin %i to %i\n",pin,val);
