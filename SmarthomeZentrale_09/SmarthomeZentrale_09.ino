...
#define TFT_LED  15
#define TOUCH_CS 14
#define TOUCH_IRQ 2 //Für AZ-Touch alte Version #define TOUCH_IRQ 2 neue Version 27
#define TOUCH_ROTATION 1 //Muss für 2.4 Zoll Display 1 und 2.8 Zoll 3 sein
...
char statpraefix[60] = "stat,confm"; //Präfix für MQTT Status Nachrichten
...
uint32_t dispOffTime = 0; //Zeitstempel für automatische Dunkelschaltung
...
void checkMQTT() {
  ...
  if (!client.connected() && (millis() > nextConnect)) {
    ...
    if (con) {
      ...
      char tp[50];
      char tmp[200];
      //mehrere Komma getrennte Präfixe sind möglich
      char * p;
      strncpy(tmp,statpraefix,200);//muss kopiert werden, da strtok den Inhalt verändert
      p=strtok(tmp,",");
      while (p != NULL) {
        strcpy(tp,p); strcat(tp,"/#");
        client.subscribe(tp);
        Serial.printf("Stat Praefix %s subscribe %s\n",p,tp);
        p = strtok(NULL,",");
      }
      Serial.println("Done");
      if (client.connected()) {
      ...
      if (state < 0) {
        Serial.print("Netzwerkprobleme, rc=");
...
void setConf(String json) {
    ...
    if (doc.containsKey("statpraefix")) strlcpy(statpraefix, doc["statpraefix"],120);
    ...
    if (doc.containsKey("displayoff"))displayoff = doc["displayoff"];
}
...
void onMQTTData(char* topic, byte* payload, unsigned int length) {
  ...
  if (strncmp(topic,"confm/",5) != 0) {
    ...
    wort = strtok(topic,"/"); //conf
    if (wort != NULL) wort = strtok(NULL,"/");
    if ((wort != NULL) && (strcmp(wort,hostname) == 0)){
      cmd = strtok(NULL,"/");
      wort = strtok(NULL,"/");
      rule = (wort == NULL)?-1:String(wort).toInt();
      wort = strtok(NULL,"/");
      element = (wort == NULL)?-1:String(wort).toInt();
      if (cmd != NULL) atm.mqttConfig(cmd,rule,element,(char *)payload);
    }
  }
}
...
bool publish(const char * thema, const char * message) {
  ...
  boolean retain = true;
  if (client.connected()) {
    if (strncmp(thema,"confs/",5) != 0) {
      strcpy(tp,cmndpraefix); strcat(tp,"/");strcat(tp,thema);
    } else {
      retain = false;
      sprintf(tp,thema,hostname);
    }
    Serial.printf("MQTT send %s -> %s\n",tp,message);
    client.publish(tp,message,retain);
    return true;
  } else {
    return false;
  }
...
void readESPNow(const uint8_t *mac_addr, const uint8_t *r_data, int data_len) {
  ...
  if (data_len < 256) {
    ...
    char * pos = strchr(buf,'{');
    if (pos) {
      //wenn die Nachricht einen JSON-String enthält
      char topic[50];
      ...
      char * pos = strchr(buf,'|');
      if (pos) {
        //wenn die Nachricht ein Kommando enthält
        char topic[50];
        strlcpy(topic,buf,pos-buf+1);
        int16_t index = nowAlias.findOriginal(String(topic));
        if (index >= 0) {
          String alias = nowAlias.getAlias(index);
          if (alias != "") client.publish(alias.c_str(),pos+1);
        } else {
          nowAlias.addEntry(String(topic));
        }
      }
...
void loop() {
  server.handleClient();
  if ((displayoff != 0) && ((millis()-dispOffTime) > (displayoff*1000))) {
    displayLed(false); 
    screenOff = true;
  }
  ArduinoOTA.handle();
